...
var app = angular.module('tea', ['ionic']);
...
app.controller('detailCtrl',
  ['$scope', '$stateParams', '$customHttp', '$ionicPopup','$rootScope','$state',
    function ($scope, $stateParams, $customHttp, $ionicPopup,$rootScope,$state) {
      ...
      $scope.addToCart = function () {
        if($rootScope.uid){
        $customHttp.get(
          'data/cart_update.php?uid='+$rootScope.uid+'&did=' + $scope.dish.did + "&count=-1",
          function (data) {
            console.log(data);
            if (data.msg == 'succ') {
              //当添加到购物车成功时，总数肯定是自增
...
app.controller('myOrderCtrl',
  ['$scope', '$customHttp','$state','$rootScope',
    function ($scope, $customHttp,$state,$rootScope) {
      //var userid = sessionStorage.getItem('userid');
      if($rootScope.uid) {
        $customHttp.get(
            'data/order_getbyuserid.php?userid=' + $rootScope.uid,
...
app.controller('cartCtrl',
  ['$scope', '$customHttp','$rootScope',
    function ($scope, $customHttp,$rootScope) {
      ...
      //获取购物车中的内容
      $customHttp.get(
        'data/cart_select.php?uid='+$rootScope.uid,
        function (dataFromServer) {
      ...
      $scope.add = function (index) {
        ...
        $customHttp.get(
          'data/cart_update.php?uid='+$rootScope.uid+'&did='
